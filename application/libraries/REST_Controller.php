...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    protected function _detect_api_key()
    {
        ...
        $this->rest->key_status = null;

        // Find the key from server or arguments
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $api_key_rplcd)) {
			...
			//$this->_detect_key_status();
            
            isset($row->user_id) and $this->rest->user_id = $row->user_id;
            isset($row->level) and $this->rest->level = $row->level;
            isset($row->ignore_limits) and $this->rest->ignore_limits = $row->ignore_limits;
            isset($row->status) and $this->rest->key_status = $row->status;

            $this->_apiuser =  $row;
            
            if ($row->status == 0)
            {
            	// Respond to key status
            	$response = array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => 'This API key has been disabled.');
            	$this->response($response, 401);
            }
    ...
    protected function _log_request($authorized = false)
    {
        $status = $this->rest->db->insert(config_item('rest_logs_table'), array(
                    ...
                    'params' => $this->_args ? (config_item('rest_logs_json_params') ? json_encode($this->_args, JSON_PRETTY_PRINT) : serialize($this->_args)) : null,
